...
- E2E 커버리지를 추가하거나 변경할 때는 ≥1개의 성공 경로와 ≥1개의 실패 경로를 포함한다.
- 문서/설정만 변경한 경우에는 런타임 테스트가 필요하지 않은 이유를 설명한다.
- 동시성/락/재시도에서 비롯될 위험(중복, 데드락 등)을 선제적으로 평가한다.

## 커밋 규칙 (Commit Rules)

- 커밋 제목은 `feat:`, `fix:`, `docs:`, `test:`, `refactor:`, `build:`, `ci:`, `chore:` 같은 Conventional Commit 타입 접두사를 사용해야 한다.
- Conventional Commit 접두사 뒤의 커밋 제목은 사용자가 다른 언어를 명시적으로 요청하지 않는 한 기본적으로 한국어로 작성한다.
- 사용자가 명시적으로 요청하지 않는 한 커밋을 생성, 수정, 푸시하지 않는다.
- 커밋 본문은 트레일러 앞에 변경 사항을 일반 불릿 목록으로 적는다.
- 커밋 본문의 불릿 항목은 연속해서 작성하고, 불릿 항목 사이에는 빈 줄을 넣지 않는다.
- 여러 줄짜리 커밋 본문은 인접한 불릿 줄을 보존하는 커밋 메시지 파일이나 단일 본문 인자를 사용하고, 각 불릿을 별도의 `-m` 문단으로 넘기지 않는다.
- Codex가 커밋을 생성하거나 수정할 때는 기본적으로 `Co-Authored-By` 트레일러를 포함한다.
- 요청된 모델명이나 활성 모델명을 명확히 알 수 있으면, 예를 들어 `Co-Authored-By: GPT 5.5 <codex@openai.com>`처럼 사용한다.
- 요청된 모델명이나 활성 모델명이 보이지 않으면 모델 버전을 추측하지 말고 `Co-Authored-By: Codex <codex@openai.com>`를 사용한다.
- 사용자가 명시적으로 요청한 경우에만 트레일러를 생략한다.
